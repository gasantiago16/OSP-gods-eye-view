...
1. **`osp-twins` layer** — Boca→Maui POC is the `osp-twin` data layer (`/osp/boca-maui.json`), a recorded `run_ptp` CATCH polyline labeled `OSP ECI TRUTH`. Falcon 9 circular-LEO is still later. This does not replace the cinematic Launch Library replay; that stays `RECONSTRUCTED ESTIMATE`.
...
3. **Pad camera recipes** — Boca, KSC, VAFB, and Maui chips on the OSP Twin row, plus `#pad=boca` (and `ksc`, `vafb`, `maui`). Coordinates are the OSP site LLA. An explicit `lat`/`lon` in the same hash still wins.
4. **Honesty HUD** — selecting the OSP twin line shows pack id, miss, LLA, and ECEF derived from the sample. ECI is not shown: this pack has no absolute epoch. Launch Library replay stays `RECONSTRUCTED ESTIMATE`.
