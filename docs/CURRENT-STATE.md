...
- **Token flow**: browser fetches a short-lived client secret from `/api/realtime/token`; the Vite middleware holds `XAI_API_KEY` (Graph Hockey SpaceXAI account) and mints `api.x.ai/v1/realtime/client_secrets`. The client opens `wss://api.x.ai/v1/realtime` with `xai-client-secret.<token>` and sends `session.update` (instructions + GEV tools). HUD summaries use `POST /v1/chat/completions` with `grok-4.6`.
...
- Layer id `weather-radar`, share token `n` (absent means off). Absent-token opacity is 65% (`lo` omitted). Bing Aerial and Bing Labels present at 40% until an opacity chip is chosen; a chosen 65% is stored as `opacityChosen` (`lo=n.c.1`) so reload does not snap it back to 40%. FIRMS keeps token `w`.
...
- Weather flags HUD (`#weather-flags-hud`) samples World Track MRMS at the camera via `/api/osp-world`. Human ACK/DISMISS only. Unavailable World Track is not SAFE. Does not write OSP trajectories.
