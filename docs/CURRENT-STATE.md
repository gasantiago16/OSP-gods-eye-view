...
- Layer id `weather-radar`, share token `n` (absent means off). Absent-token opacity is 65% (`lo` omitted). Bing Aerial and Bing Labels present at 40% until an opacity chip is chosen; a chosen 65% is stored as `opacityChosen` (`lc=1`) so reload does not snap it back to 40%. FIRMS keeps token `w`.
