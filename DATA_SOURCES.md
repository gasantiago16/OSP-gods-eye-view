...
| **RainViewer** | Live composite weather radar overlay (`weather-radar`) | Personal/educational use; 2026 public radar is past ~2 hours at ~10 min, zoom 7, Universal Blue only; 100 requests/IP/minute. Nowcast and satellite IR were discontinued 2026-01-01. [API](https://www.rainviewer.com/api.html), [transition notice](https://www.rainviewer.com/api/transition-faq.html) | Linked "RainViewer" in the Data attribution popover |
| **NOAA MRMS** (via OSP World Track) | Numeric composite reflectivity + Stage A weather flags HUD | NODD public use; attribution requested. CONUS mosaic; off-grid is UNKNOWN not “no rain.” Decision support only, not ATC. | Linked "NOAA MRMS" in the Data attribution popover |
...
- **Weather radar.** `weather-radar` is a Cesium imagery overlay of RainViewer composite reflectivity, fetched through `/api/weather-radar/catalog` and `/api/weather-radar/tiles/{frame}/{z}/{x}/{y}.png`. The browser never chooses an upstream URL. Tiles are Universal Blue, max zoom 7, and only advertised catalog frames. Empty/transparent tiles are not clear skies — coverage is not global. Google Photoreal 3D hides the globe, so enabling radar switches to Bing Aerial when ion imagery is available, otherwise OSM. Playback walks past frames only; nowcast is discontinued. This is independent of cockpit Open-Meteo effects.
- **Weather flags.** GEV does not ingest MRMS. `/api/osp-world/weather` and `/api/osp-world/weather/flags` proxy loopback World Track (`:8765`) only. The DATA LAYERS panel shows dBZ at the camera and pending flags for human ACK/DISMISS. RainViewer remains the picture; flags are not a go/no-go.
