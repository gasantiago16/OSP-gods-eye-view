...
|-----|----------------|--------------------------|
| `XAI_API_KEY` | Server only | HUD uses `POST /v1/chat/completions`. Browser fetches a short-lived **ephemeral** Grok Voice token from `/api/realtime/token`; the real key never ships |
